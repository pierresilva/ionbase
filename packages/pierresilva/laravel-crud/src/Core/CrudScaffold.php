    ...
    public function setupFrontPage()
    {
        ...
        foreach ($this->data->models as $model) {
            ...
            if (!$model->is_pivot && $model->name != '') {
                ...
                $output_dir = base_path() . '/src/app/' . NameResolver::solveName($model->name, 'name-names');
                ...
                $stub_txt = $this->files->get(__DIR__ . '/../Stubs/src/app/page/html.stub');
                $output_filename = NameResolver::solveName($model->name, 'name-names') . '.page.html';
                $output_path = $output_dir . '/' . $output_filename;
                $stub_obj = new StubCompiler($stub_txt, $model);
                $output = $stub_obj->compile();
                ...
                $stub_txt = $this->files->get(__DIR__ . '/../Stubs/src/app/page/scss.stub');
                $output_filename = NameResolver::solveName($model->name, 'name-names') . '.page.scss';
                $output_path = $output_dir . '/' . $output_filename;
                $stub_obj = new StubCompiler($stub_txt, $model);
                $output = $stub_obj->compile();
                ...
                $stub_txt = $this->files->get(__DIR__ . '/../Stubs/src/app/page/module_ts.stub');
                ...
                $stub_txt = $this->files->get(__DIR__ . '/../Stubs/src/app/page/model_ts.stub');
                ...
                $stub_txt = $this->files->get(__DIR__ . '/../Stubs/src/app/page/service.stub');
                $output_filename = NameResolver::solveName($model->name, 'name-names') . '.service.ts';
                $output_path = $output_dir . '/' . $output_filename;
                $stub_obj = new StubCompiler($stub_txt, $model);
                $output = $stub_obj->compile();
                ...
                // list
                // list html
                $stub_txt = $this->files->get(__DIR__ . '/../Stubs/src/app/page/list/html.stub');
                $output_filename = NameResolver::solveName($model->name, 'name-names') . '-list.component.html';
                $output_path = $output_dir . '/' . NameResolver::solveName($model->name, 'name-names') . '-list/' . $output_filename;
                $stub_obj = new StubCompiler($stub_txt, $model);
                $output = $stub_obj->compile();
                ...
                if (!$this->files->exists($output_dir . '/' . NameResolver::solveName($model->name, 'name-names') . '-create')) {
                    $this->files->makeDirectory($output_dir . '/' . NameResolver::solveName($model->name, 'name-names') . '-create', $mode = 0777, $recursive = false, $force = false);
                }
                // create html
                $stub_txt = $this->files->get(__DIR__ . '/../Stubs/src/app/page/create/html.stub');
                $output_filename = NameResolver::solveName($model->name, 'name-names') . '-create.component.html';
                $output_path = $output_dir . '/' . NameResolver::solveName($model->name, 'name-names') . '-create/' . $output_filename;
                $stub_obj = new StubCompiler($stub_txt, $model);
                $output = $stub_obj->compile();
                ...
                $output_filename = NameResolver::solveName($model->name, 'name-names') . '-create.component.ts';
                $output_path = $output_dir . '/' . NameResolver::solveName($model->name, 'name-names') . '-create/' . $output_filename;
                $stub_obj = new StubCompiler($stub_txt, $model);
                $output = $stub_obj->compile();
                ...
                $output_filename = NameResolver::solveName($model->name, 'name-names') . '-create.component.scss';
                $output_path = $output_dir . '/' . NameResolver::solveName($model->name, 'name-names') . '-create/' . $output_filename;
                $stub_obj = new StubCompiler($stub_txt, $model);
                $output = $stub_obj->compile();
                ...
                if (!$this->files->exists($output_dir . '/' . NameResolver::solveName($model->name, 'name-names') . '-edit')) {
                    $this->files->makeDirectory($output_dir . '/' . NameResolver::solveName($model->name, 'name-names') . '-edit', $mode = 0777, $recursive = false, $force = false);
                }
                // edit html
                $stub_txt = $this->files->get(__DIR__ . '/../Stubs/src/app/page/edit/html.stub');
                $output_filename = NameResolver::solveName($model->name, 'name-names') . '-edit.component.html';
                $output_path = $output_dir . '/' . NameResolver::solveName($model->name, 'name-names') . '-edit/' . $output_filename;
                $stub_obj = new StubCompiler($stub_txt, $model);
                $output = $stub_obj->compile();
                ...
                $output_filename = NameResolver::solveName($model->name, 'name-names') . '-edit.component.ts';
                $output_path = $output_dir . '/' . NameResolver::solveName($model->name, 'name-names') . '-edit/' . $output_filename;
                $stub_obj = new StubCompiler($stub_txt, $model);
                $output = $stub_obj->compile();
                ...
                $output_filename = NameResolver::solveName($model->name, 'name-names') . '-edit.component.scss';
                $output_path = $output_dir . '/' . NameResolver::solveName($model->name, 'name-names') . '-edit/' . $output_filename;
                $stub_obj = new StubCompiler($stub_txt, $model);
                $output = $stub_obj->compile();
