...
@if(isset($compResponsability))
@if($errors->any()){{ old('model.name') }}@else{{ $compResponsability->name ?? '' }}@endif
@endif" required>
...
<div class="form-group">
  ...
  @foreach( $lists["Company"] as $list_key => $list_item )
    <option value="{{ $list_key }}"
      ...
      @else
        @if( isset($compResponsability) && $compResponsability->company_id==$list_key )  selected='selected' @endif
      @endif
   >{{ $list_item }}</option>
  @endforeach
  </select>
...
<div class="form-group">
  ...
  @foreach( $lists["SystParameter"] as $list_key => $list_item )
    <option value="{{ $list_key }}"
      ...
      @else
        @if( isset($compResponsability) && $compResponsability->syst_parameter_id==$list_key )  selected='selected' @endif
      @endif
   >{{ $list_item }}</option>
  @endforeach
  </select>
