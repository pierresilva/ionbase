              ...
              <nav aria-label="breadcrumb">
                <ol class="breadcrumb">
                  ...
                  @if ( $query_params = Request::input('q') )
                      ...
                      <li class="breadcrumb-item active"><a href="{{ route('housingUnitAreas.index') }}">{{ $model_title_list['housingUnitAreas'] }}</a></li>
                  ...
                  @else
                      <li class="breadcrumb-item active">{{ $model_title_list['housingUnitAreas'] }}</li>
                  @endif
              ...
              <div class="card">
                  <div class="card-body">
                      <h1 class="d-flex mb-3">
                          ...
                          <span class="d-inline-block">{{ $model_title_list['housingUnitAreas'] }}</span>
                          <a class="btn btn-success d-inline-flex ml-auto" href="{{ route('housingUnitAreas.create') }}"><i class="material-icons align-self-center mr-1">add_circle</i><span class="align-self-center">Create</span></a>
                      </h1>
                      ...
                      <div class="collapse mb-4" id="search-area">
                        <div class="card card-body">
                          ...
                          <form id="search" action="{{ route('housingUnitAreas.index') }}" accept-charset="UTF-8" method="get">
                            ...
                            <div class="form-group row">
                              ...
                              <div class="col-sm-10">
                                <input class="form-control form-control-sm" id="q_code_cont" name="q[code:cont]" type="search" value="{{ @(Request::input('q.code:cont')) }}">
                                                                                    ...
                                                                                    <div class="form-group row">
                              <label for="q_housing_unit_id_eq" class="col-sm-2 col-form-label col-form-label-sm">UNIDADES HABITACIONALES<br>belongsTo</label>
                              <div class="col-sm-10">
                                {{--<input class="form-control form-control-sm" id="q_housing_unit_id_eq" name="q[housing_unit_id:eq]" type="search" value="{{ @(Request::input('q.housing_unit_id:eq')) }}">--}}
                                <select class="form-control"
                                    id="q_housing_unit_id_eq"
                                    name="q[housing_unit_id:eq]">
                                <option value=""></option>
                                @foreach( $lists["HousingUnit"] as $list_key => $list_item )
                                    <option value="{{ $list_key }}"
                                            @if( $list_key == @(Request::input('q.housing_unit_id:eq')) )  selected='selected' @endif
                                    >{{ $list_item }}</option>
                                @endforeach
                                </select>
                              </div>
                      ...
                      @include('housingUnitAreas._table')
                  </div>
              </div>
          </div>
      </div>
