...
@if(isset($housingUnitArea))
@if($errors->any()){{ old('model.name') }}@else{{ $housingUnitArea->name ?? '' }}@endif
@endif" required>
...
@if(isset($housingUnitArea))
@if($errors->any()){{ old('model.code') }}@else{{ $housingUnitArea->code ?? '' }}@endif
@endif" required>
...
<div class="form-group">
  ...
  @foreach( $lists["HousingUnit"] as $list_key => $list_item )
    <option value="{{ $list_key }}"
      ...
      @else
        @if( isset($housingUnitArea) && $housingUnitArea->housing_unit_id==$list_key )  selected='selected' @endif
      @endif
   >{{ $list_item }}</option>
  @endforeach
  </select>
