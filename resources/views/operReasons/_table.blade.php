    ...
    <div class="col-md-12">
        ...
        @if($operReasons->count())
            ...
            <table class="table table-sm table-striped sp-omit">
              ...
              <tbody>
                @foreach($operReasons as $operReason)
                    <tr>
                      <td scope="row"><a href="{{ route('operReasons.show', $operReason->id) }}">{{$operReason->id}}</a></td>
                      <td>{{$operReason->name}}</td>


                      <td>
                          @foreach($operReason->operCorrectiveMaintenances as $operCorrectiveMaintenance)
                                        @if (!$loop->first) , @endif
                                        <a href="{{ route('operCorrectiveMaintenances.show', $operCorrectiveMaintenance->id) }}">{{ $operCorrectiveMaintenance->name }}</a>
                          @endforeach
                      </td>
                      ...
                      <td class="text-right">
                        <div class="btn-group" role="group">
                            ...
                            <form method="POST" action="{{ route('operReasons.destroy', $operReason->id) }}" accept-charset="UTF-8" style="display: inline;" onsubmit="if(confirm('Delete? Are you sure?')) { return true } else {return false };">
                                {{ method_field('DELETE') }}
                                {{ csrf_field() }}
            ...
            @if( method_exists($operReasons, 'appends') )
              {!! $operReasons->appends(Request::except('page'))->render() !!}
            @endif
        @else
