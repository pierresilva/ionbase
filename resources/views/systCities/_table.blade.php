    ...
    <div class="col-md-12">
        ...
        @if($systCities->count())
            ...
            <table class="table table-sm table-striped sp-omit">
              <thead>
                <tr>
                  <th scope="col"><div class="d-flex">
                    @if( method_exists($systCities, 'appends') )
              ...
              <tbody>
                @foreach($systCities as $systCity)
                    <tr>
                      <td scope="row"><a href="{{ route('systCities.show', $systCity->id) }}">{{$systCity->id}}</a></td>
                      <td>{{$systCity->name}}</td>
                      <td>{{$systCity->code}}</td>

                      <td>@if($systCity->systRegion)<a href="{{ route('systRegions.show', $systCity->systRegion->id) }}">{{ $systCity->systRegion->name }}</a>@else - @endif</td>

                      <td>
                          @foreach($systCity->userProfiles as $userProfile)
                                        @if (!$loop->first) , @endif
                                        <a href="{{ route('userProfiles.show', $userProfile->id) }}">{{ $userProfile->name }}</a>
                          @endforeach
                      </td>
                      <td>
                          @foreach($systCity->companyAddresses as $companyAddress)
                                        @if (!$loop->first) , @endif
                      ...
                      <td class="text-right">
                        <div class="btn-group" role="group">
                            ...
                            <form method="POST" action="{{ route('systCities.destroy', $systCity->id) }}" accept-charset="UTF-8" style="display: inline;" onsubmit="if(confirm('Delete? Are you sure?')) { return true } else {return false };">
                                {{ method_field('DELETE') }}
                                {{ csrf_field() }}
            ...
            @if( method_exists($systCities, 'appends') )
              {!! $systCities->appends(Request::except('page'))->render() !!}
            @endif
        @else
