            ...
            <table class="table table-sm table-striped sp-omit">
              <thead>
                <tr>
                  ...
                  <th scope="col">DIRECCIONES DE EMPRESAS</th>
                  <th scope="col">CONTACTOS DE EMPRESAS</th>
                  <th scope="col">JUNTA DIRECTIVA</th>
                  <th scope="col">RESPONSABILIDADES LEGALES</th>
              ...
              <tbody>
                @foreach($companies as $company)
                    <tr>
                      ...
                      <td>
                          @foreach($company->companyPhones as $companyPhone)
                                        @if (!$loop->first) , @endif
                                        <a href="{{ route('companyPhones.show', $companyPhone->id) }}">{{ $companyPhone->name }}</a>
                          @endforeach
                      </td>
                      <td>
                          @foreach($company->companyAddresses as $companyAddress)
                                        @if (!$loop->first) , @endif
                                        <a href="{{ route('companyAddresses.show', $companyAddress->id) }}">{{ $companyAddress->name }}</a>
                          @endforeach
                      </td>
                      <td>
                          @foreach($company->companyContacts as $companyContact)
                                        @if (!$loop->first) , @endif
                                        <a href="{{ route('companyContacts.show', $companyContact->id) }}">{{ $companyContact->name }}</a>
                          @endforeach
                      </td>
                      <td>
                          @foreach($company->companyDirectorates as $companyDirectorate)
                                        @if (!$loop->first) , @endif
                                        <a href="{{ route('companyDirectorates.show', $companyDirectorate->id) }}">{{ $companyDirectorate->name }}</a>
                          @endforeach
                      </td>
                      <td>
                          @foreach($company->companyLegalResponsabilities as $companyLegalResponsability)
                                        @if (!$loop->first) , @endif
                                        <a href="{{ route('companyLegalResponsabilities.show', $companyLegalResponsability->id) }}">{{ $companyLegalResponsability->name }}</a>
                          @endforeach
                      </td>
