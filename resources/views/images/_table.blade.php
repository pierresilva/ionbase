            ...
            <table class="table table-sm table-striped sp-omit">
              <thead>
                <tr>
                  <th scope="col"><div class="d-flex">
                    @if( method_exists($images, 'appends') )
              ...
              <tbody>
                @foreach($images as $image)
                    <tr>
                      <td scope="row"><a href="{{ route('images.show', $image->id) }}">{{$image->id}}</a></td>
                      <td>{{$image->name}}</td>
                      <td>{{$image->url}}</td>
                      ...
                      <td class="text-right">
                        <div class="btn-group" role="group">
                            ...
                            <form method="POST" action="{{ route('images.destroy', $image->id) }}" accept-charset="UTF-8" style="display: inline;" onsubmit="if(confirm('Delete? Are you sure?')) { return true } else {return false };">
                                {{ method_field('DELETE') }}
                                {{ csrf_field() }}
