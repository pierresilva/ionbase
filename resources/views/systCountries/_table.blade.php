    ...
    <div class="col-md-12">
        ...
        @if($systCountries->count())
            ...
            <table class="table table-sm table-striped sp-omit">
              ...
              <tbody>
                @foreach($systCountries as $systCountry)
                    <tr>
                      <td scope="row"><a href="{{ route('systCountries.show', $systCountry->id) }}">{{$systCountry->id}}</a></td>
                      <td>{{$systCountry->name}}</td>
                      <td>{{$systCountry->code}}</td>


                      <td>
                          @foreach($systCountry->userProfiles as $userProfile)
                                        @if (!$loop->first) , @endif
                                        <a href="{{ route('userProfiles.show', $userProfile->id) }}">{{ $userProfile->name }}</a>
                          @endforeach
                      </td>
                      <td>
                          @foreach($systCountry->systRegions as $systRegion)
                                        @if (!$loop->first) , @endif
                                        <a href="{{ route('systRegions.show', $systRegion->id) }}">{{ $systRegion->name }}</a>
                          @endforeach
                      </td>
                      ...
                      <td class="text-right">
                        <div class="btn-group" role="group">
                            ...
                            <form method="POST" action="{{ route('systCountries.destroy', $systCountry->id) }}" accept-charset="UTF-8" style="display: inline;" onsubmit="if(confirm('Delete? Are you sure?')) { return true } else {return false };">
                                {{ method_field('DELETE') }}
                                {{ csrf_field() }}
            ...
            @if( method_exists($systCountries, 'appends') )
              {!! $systCountries->appends(Request::except('page'))->render() !!}
            @endif
        @else
