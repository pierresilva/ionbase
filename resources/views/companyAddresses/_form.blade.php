...
@if($errors->any()){{ old('model.name') }}@else{{ $companyAddress->name ?? '' }}@endif
@endif" required>
...
@if(isset($companyAddress))
@if($errors->any()){{ old('model.address') }}@else{{ $companyAddress->address ?? '' }}@endif
@endif" required>
...
<div class="form-group">
  ...
  @foreach( $lists["Company"] as $list_key => $list_item )
    <option value="{{ $list_key }}"
      ...
      @else
        @if( isset($companyAddress) && $companyAddress->company_id==$list_key )  selected='selected' @endif
      @endif
   >{{ $list_item }}</option>
  @endforeach
  </select>
...
<div class="form-group">
  ...
  @foreach( $lists["SystCity"] as $list_key => $list_item )
    <option value="{{ $list_key }}"
      ...
      @else
        @if( isset($companyAddress) && $companyAddress->syst_city_id==$list_key )  selected='selected' @endif
      @endif
   >{{ $list_item }}</option>
  @endforeach
  </select>
