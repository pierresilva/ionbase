...
@if($errors->any()){{ old('model.name') }}@else{{ $companyPhone->name ?? '' }}@endif
@endif" required>
...
@if(isset($companyPhone))
@if($errors->any()){{ old('model.number') }}@else{{ $companyPhone->number ?? '' }}@endif
@endif" required>
...
<div class="form-group">
  ...
  @foreach( $lists["Company"] as $list_key => $list_item )
    <option value="{{ $list_key }}"
      ...
      @else
        @if( isset($companyPhone) && $companyPhone->company_id==$list_key )  selected='selected' @endif
      @endif
   >{{ $list_item }}</option>
  @endforeach
  </select>
