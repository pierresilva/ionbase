<div class="row">
    <div class="col-md-12">
        ...
        @if($companyPhones->count())
            ...
            <table class="table table-sm table-striped sp-omit">
              ...
              <tbody>
                @foreach($companyPhones as $companyPhone)
                    <tr>
                      <td scope="row"><a href="{{ route('companyPhones.show', $companyPhone->id) }}">{{$companyPhone->id}}</a></td>
                      <td>{{$companyPhone->name}}</td>
                      <td>{{$companyPhone->number}}</td>
                      <td>{{$companyPhone->main}}</td>

                      <td>@if($companyPhone->company)<a href="{{ route('companies.show', $companyPhone->company->id) }}">{{ $companyPhone->company->name }}</a>@else - @endif</td>
                      ...
                      <td class="text-right">
                        <div class="btn-group" role="group">
                            ...
                            <form method="POST" action="{{ route('companyPhones.destroy', $companyPhone->id) }}" accept-charset="UTF-8" style="display: inline;" onsubmit="if(confirm('Delete? Are you sure?')) { return true } else {return false };">
                                {{ method_field('DELETE') }}
                                {{ csrf_field() }}
            ...
            @if( method_exists($companyPhones, 'appends') )
              {!! $companyPhones->appends(Request::except('page'))->render() !!}
            @endif
        @else
