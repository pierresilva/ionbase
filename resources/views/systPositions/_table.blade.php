    ...
    <div class="col-md-12">
        ...
        @if($systPositions->count())
            ...
            <table class="table table-sm table-striped sp-omit">
              ...
              <tbody>
                @foreach($systPositions as $systPosition)
                    <tr>
                      <td scope="row"><a href="{{ route('systPositions.show', $systPosition->id) }}">{{$systPosition->id}}</a></td>
                      <td>{{$systPosition->name}}</td>
                      <td>{{$systPosition->code}}</td>


                      <td>
                          @foreach($systPosition->users as $user)
                                        @if (!$loop->first) , @endif
                                        <a href="{{ route('users.show', $user->id) }}">{{ $user->name }}</a>
                          @endforeach
                      </td>
                      <td>
                          @foreach($systPosition->companyContacts as $companyContact)
                                        @if (!$loop->first) , @endif
                                        <a href="{{ route('companyContacts.show', $companyContact->id) }}">{{ $companyContact->name }}</a>
                          @endforeach
                      </td>
                      <td>
                          @foreach($systPosition->companyDirectorates as $companyDirectorate)
                                        @if (!$loop->first) , @endif
                      ...
                      <td class="text-right">
                        <div class="btn-group" role="group">
                            ...
                            <form method="POST" action="{{ route('systPositions.destroy', $systPosition->id) }}" accept-charset="UTF-8" style="display: inline;" onsubmit="if(confirm('Delete? Are you sure?')) { return true } else {return false };">
                                {{ method_field('DELETE') }}
                                {{ csrf_field() }}
            ...
            @if( method_exists($systPositions, 'appends') )
              {!! $systPositions->appends(Request::except('page'))->render() !!}
            @endif
        @else
