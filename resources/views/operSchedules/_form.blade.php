...
<div class="form-group">
  <label for="hous_unit_area_id-field">AREAS DE UNIDADES HABITACIONALES</label>
  ...
  @foreach( $lists["HousUnitArea"] as $list_key => $list_item )
    <option value="{{ $list_key }}"
      ...
      @else
        @if( isset($operSchedule) && $operSchedule->hous_unit_area_id==$list_key )  selected='selected' @endif
      @endif
   >{{ $list_item }}</option>
  @endforeach
  </select>
