...
@if(isset($compContact))
@if($errors->any()){{ old('model.name') }}@else{{ $compContact->name ?? '' }}@endif
@endif" required>
...
@if(isset($compContact))
@if($errors->any()){{ old('model.phone') }}@else{{ $compContact->phone ?? '' }}@endif
@endif" required>
...
@if(isset($compContact))
@if($errors->any()){{ old('model.email') }}@else{{ $compContact->email ?? '' }}@endif
@endif" >
...
<div class="form-group">
  ...
  @foreach( $lists["Company"] as $list_key => $list_item )
    <option value="{{ $list_key }}"
      ...
      @else
        @if( isset($compContact) && $compContact->company_id==$list_key )  selected='selected' @endif
      @endif
   >{{ $list_item }}</option>
  @endforeach
  </select>
...
<div class="form-group">
  ...
  @foreach( $lists["SystPosition"] as $list_key => $list_item )
    <option value="{{ $list_key }}"
      ...
      @else
        @if( isset($compContact) && $compContact->syst_position_id==$list_key )  selected='selected' @endif
      @endif
   >{{ $list_item }}</option>
  @endforeach
  </select>
